...
BASE_DIR = Path('/mnt/data/ersaal-catalog-site')
DIST = BASE_DIR / 'dist'
ASSETS = DIST / 'assets'
CSS_DIR = ASSETS / 'css'
JS_DIR = ASSETS / 'js'
...
CSV_PATH = Path('/mnt/data/Beckhoff1-500_Done test.csv')
...
def load_products() -> list[dict]:
    ...
    raw = raw.replace(bytes([129]), 'ü'.encode('utf-8')).replace(bytes([211]), 'ö'.encode('utf-8')).replace(bytes([227]), 'ä'.encode('utf-8'))
    df = pd.read_csv(io.BytesIO(raw), encoding='utf-8')
    products = []
    seen = set()
    for i, row in df.iterrows():
        name = clean_text(row.get('Title')) or clean_text(row.get('Handle')) or clean_text(row.get('Variant SKU'))
        sku = clean_text(row.get('Variant SKU')) or clean_text(row.get('Handle'))
        brand = clean_text(row.get('Vendor')) or 'ERSAAL'
        category = clean_text(row.get('Type')) or clean_text(row.get('Product Category')) or 'Industriekomponenten'
        top_category = clean_text(row.get('Product Category')) or 'Industriekomponenten'
        ...
        seo_title = clean_text(row.get('SEO Title')) or name
        seo_description = clean_text(row.get('SEO Description')) or description
        image = clean_text(row.get('Image Src')) or clean_text(row.get('Variant Image'))
        tags = [t.strip() for t in clean_text(row.get('Tags')).split(',') if t.strip()]
        condition = clean_text(row.get('Google Shopping / Condition')) or clean_text(row.get('Option1 Value')) or 'Neu / Gebraucht'
        mpn = clean_text(row.get('Google Shopping / MPN')) or sku
        handle = clean_text(row.get('Handle')) or sku or name
        slug = slugify(f"{brand}-{sku or handle}")
        unique = slug
        n = 2
        ...
        product = {
            ...
            'inventory_qty': clean_text(row.get('Variant Inventory Qty')),
            'alt': clean_text(row.get('Image Alt Text')) or name,
        }
...
def product_card(p, depth=0):
    root = rel_path(depth)
    image = html.escape(p['image']) if p['image'] else f"{root}assets/img/product-placeholder.svg"
    img_tag = f'<img src="{image}" alt="{html.escape(p["alt"])}" loading="lazy">'
    return f'''<article class="product-card">
      <a class="product-card__image" href="{root[:-1] if root.endswith('/') else root}{p['url'] if depth==0 else p['url']}">{img_tag}</a>
      <div class="product-card__body">
        <div class="eyebrow">{html.escape(p['brand'])}</div>
        <h3><a href="{root[:-1] if root.endswith('/') else root}{p['url'] if depth==0 else p['url']}">{html.escape(p['name'])}</a></h3>
        <p class="sku">SKU: {html.escape(p['sku'])}</p>
        <div class="product-card__actions">
          <a class="btn btn-primary" href="{root[:-1] if root.endswith('/') else root}{p['url'] if depth==0 else p['url']}#quote-form">Angebot anfordern</a>
          <a class="text-link" href="{root[:-1] if root.endswith('/') else root}{p['url'] if depth==0 else p['url']}">Weiterlesen »</a>
        </div>
      </div>
    </article>'''
...
def make_product_jsonld(p):
    ...
    breadcrumb = {
        ...
        'itemListElement': [
            ...
            {'@type': 'ListItem', 'position': 4, 'name': p['name'], 'item': SITE_URL + p['url']},
        ]
    }
...
<main>
  ...
  <section class="section section-tight">
    <div class="container breadcrumbs"><a href="../index.html">Home</a> / <a href="../produkte/index.html">Produkte</a> / <a href="../marke/{slugify(p['brand'])}/index.html">{html.escape(p['brand'])}</a> / <span>{html.escape(p['sku'])}</span></div>
  </section>
  ...
  <section class="section section-alt">
    <div class="container">
      <div class="section-head">
        ...
        <a class="text-link" href="../marke/{slugify(p['brand'])}/index.html">Alle {html.escape(p['brand'])}-Produkte</a>
      </div>
...
'''
    ...
    out.write_text(page_shell(page_title(p['seo_title']), desc, p['url'], body, depth=1, json_ld=make_product_jsonld(p)), encoding='utf-8')
...
'''
        ...
        target.write_text(page_shell(page_title(title + (f' – Seite {page_num}' if page_num > 1 else '')), meta_desc(intro), canonical, body, depth=depth), encoding='utf-8')
...
def write_home(products, brands, categories):
    ...
    brand_html = ''.join([f'<a class="pill" href="marke/{slugify(name)}/index.html">{html.escape(name)} <span>{len(items)}</span></a>' for name, items in list(brands.items())[:12]])
    body = f'''
<main>
...
'''
    ...
    (DIST / 'index.html').write_text(page_shell(page_title('Industriekomponenten & technische Beschaffung'), meta_desc('Skalierbarer B2B-Katalog für industrielle Komponenten, Ersatzteile und Angebotsanfragen.'), '/index.html', body, depth=0, json_ld=json_ld), encoding='utf-8')
...
def write_simple_page(filename, title, text, depth=1):
    ...
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(page_shell(page_title(title), meta_desc(text), f'/{filename}/index.html', body, depth=depth), encoding='utf-8')


def write_brand_index(brands):
    cards = ''.join([f'<article class="simple-card"><h2><a href="../marke/{slugify(name)}/index.html">{html.escape(name)}</a></h2><p>{len(items)} Produkte</p><a class="text-link" href="../marke/{slugify(name)}/index.html">Zur Marke »</a></article>' for name, items in brands.items()])
    ...
    (DIST / 'marken' / 'index.html').write_text(page_shell(page_title('Marken'), meta_desc('Herstellerseiten für den gesamten Produktkatalog.'), '/marken/index.html', body, depth=1), encoding='utf-8')


def write_category_index(categories):
    cards = ''.join([f'<article class="simple-card"><h2><a href="../kategorie/{slugify(name)}/index.html">{html.escape(name)}</a></h2><p>{len(items)} Produkte</p><a class="text-link" href="../kategorie/{slugify(name)}/index.html">Zur Kategorie »</a></article>' for name, items in categories.items()])
    ...
    (DIST / 'kategorien' / 'index.html').write_text(page_shell(page_title('Kategorien'), meta_desc('Kategorieseiten für Industriekomponenten.'), '/kategorien/index.html', body, depth=1), encoding='utf-8')
...
def write_search_page():
    ...
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(page_shell(page_title('Suche'), meta_desc('Produktsuche im Industriekatalog.'), '/suche/index.html', body, depth=1), encoding='utf-8')
...
## What is included
- Static HTML product pages for all {products_count} products
...
## Search strategy
Current build mode: `{'local JSON index' if local_search else 'external search recommended'}`.
...
'''
    ...
    shutil.copy2(Path('/mnt/data/build_ersaal_site.py'), BASE_DIR / 'build.py')


def main():
    ensure_dirs()
    products = load_products()
    brands = group_counts(products, 'brand')
    categories = group_counts(products, 'category')
    local_search = len(products) <= SEARCH_LOCAL_THRESHOLD

    write_assets(local_search)
    write_search_index(products, local_search)
    write_home(products, brands, categories)
    write_simple_page('kontakt', 'Kontakt', 'Senden Sie uns Ihre technische Anfrage oder Angebotsanfrage. Unser Team antwortet in der Regel innerhalb von 24 Stunden.', depth=1)
    write_simple_page('ueber-uns', 'Über Uns', 'ERSAAL unterstützt Unternehmen weltweit bei der Beschaffung schwer verfügbarer und abgekündigter Industriekomponenten.', depth=1)
    write_search_page()

    write_listing_page('Alle Produkte', 'SEO-optimierte Produktübersicht mit skalierbarer Pagination und schneller Quote-Request-Navigation.', products, DIST / 'produkte', '/produkte/index.html', depth=1, hero_title='Alle Produkte')
    write_brand_index(brands)
    write_category_index(categories)

    for brand, items in brands.items():
        write_listing_page(f'Marke {brand}', f'{len(items)} Produkte von {brand} mit statischen SEO-Landingpages und Angebotsformular.', items, DIST / 'marke' / slugify(brand), f'/marke/{slugify(brand)}/index.html', depth=2, hero_title=brand)

    for category, items in categories.items():
        write_listing_page(f'Kategorie {category}', f'{len(items)} Produkte aus der Kategorie {category}. Optimiert für organische Sichtbarkeit und interne Verlinkung.', items, DIST / 'kategorie' / slugify(category), f'/kategorie/{slugify(category)}/index.html', depth=2, hero_title=category)

    all_urls = ['/index.html', '/produkte/index.html', '/marken/index.html', '/kategorien/index.html', '/kontakt/index.html', '/ueber-uns/index.html', '/suche/index.html']
    for p in products:
        related = [x for x in brands[p['brand']] if x['slug'] != p['slug']][:3]
        write_product_page(p, related)
        all_urls.append(p['url'])
    for brand in brands:
        all_urls.append(f'/marke/{slugify(brand)}/index.html')
    for category in categories:
        all_urls.append(f'/kategorie/{slugify(category)}/index.html')

    write_sitemap(all_urls)
    write_readme(len(products), len(brands), len(categories), local_search)

    archive_base = shutil.make_archive('/mnt/data/ersaal-static-catalog', 'zip', BASE_DIR)
    print(archive_base)
